...
local function isTargetGame()
    ...
    if ok and type(info)=="table" and type(info.Name)=="string" then
        ...
        return string.find(name,"superhero",1,true)~=nil and string.find(name,"evolution",1,true)~=nil
    end
    return false
end

if not isTargetGame() then error("[SERENITY HUB] This route is only for +1 Superhero Evolution.",0) end
...
local function launch(_,proof)
    ...
    if not ok or type(source)~="string" or source=="" then error("[SERENITY HUB] +1 Superhero Evolution payload is unavailable.",0) end
...
local uiUrl=UI_URL.."?v=superhero-direct-v4&cb="..tostring(os.time())..tostring(math.random(100000,999999))
