...
local SOURCE=BASE.."dist/access-v2/monkey-provider-base.lua"
...
url=nil
if not ok or type(source)~="string" or source=="" then error("[SERENITY HUB] Monkey Access V2 source is unavailable.",0) end
local p=string.find(source,OLD,1,true)
if not p then error("[SERENITY HUB] Monkey provider marker is missing.",0) end
source=string.sub(source,1,p-1)..NEW..string.sub(source,p+#OLD)
local fn,err=loadstring(source,"@SerenityHub/AccessV2-Monkey-ProviderV3")
source=nil
if not fn then error("[SERENITY HUB] Monkey Access V2 compile failed: "..tostring(err),0) end
return fn()
